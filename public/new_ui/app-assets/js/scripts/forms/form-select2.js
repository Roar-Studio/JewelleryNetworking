...
(function (window, document, $) {
  ...
  //   })
  //   .change(function () {
  //     var form = $(this).closest('form'); // Find nearest form

  //     if (form.length) {
  //         form.validate().element($(this)); // Validate only if inside a form
  ...
  select.each(function () {
      ...
      // Wrap select in a container with a clear button
      $this.wrap('<div class="position-relative select2-container"></div>');
      ...
      $this.on('change', toggleClearButton);
  });

  $('.select2').on('select2:open', function(e) {
    let placeholderText = $(this).data('placeholder') || ''; // Get the placeholder

    let newPlaceholder = placeholderText.includes('Select') 
        ? placeholderText.replace('Select', 'Search') 
        : 'Search ' + placeholderText;
    $(document).find('.select2-search__field').attr('placeholder', newPlaceholder + '...');
  });
